...
DOCTEST_TEST_CASE("atom ion interaction") {
    ...
    system3.set_ion_distance_vector({0, 0, 1.9 * UM_IN_ATOMIC_UNITS});
    ...
    system2.set_ion_distance_vector({0, 0, 1.9 * UM_IN_ATOMIC_UNITS});
    ...
    for (size_t i = 0; i < num_energies; ++i) {
        DOCTEST_MESSAGE(fmt::format("Energies2: {}", (energies2[i] - energy) * HARTREE_IN_GHZ));
        DOCTEST_MESSAGE(fmt::format("Energies3: {}", (energies3[i] - energy) * HARTREE_IN_GHZ));
        DOCTEST_MESSAGE("--------------------");
    }
}
